...
function search(pageCode) {
	...
	request.execute(function(response) {
		...
		$.each(results.items, function(index, item) {
			...
			addVideo += '<iframe class="video w100" width="640" height="360" src="http://www.youtube.com/embed/'+videoId+'?enablejsapi=1&rel=0&showinfo=0&autohide=1&controls=1&modestbranding=1" frameborder="0" allowfullscreen></iframe>';
			addVideo += '</div>';
			addVideo += '<input type="button" class="decideURL" data-rno="http://www.youtube.com/embed/'+videoId+'" value="영상 선택"/> <br/>';
		...
		if(prevPage) {
			$("#results").append("<input type='button' id='prevPage' value='이전'/>");
		...
		if(nextPage) {
			$("#results").append("<input type='button' id='nextPage' value='다음'/>");
