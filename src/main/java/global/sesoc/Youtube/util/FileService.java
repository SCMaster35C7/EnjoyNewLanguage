...
public class FileService {
	...
	public static String saveFile(MultipartFile upload, String uploadPath) {
		...
		int lastIndex = originalFilename.lastIndexOf('.'); // 점의 위치 반환(확장자.은 가장 마지막이니 lastIndex)
		...
		while (true) {
			...
			savedFilename = filename+"_"+sdf+"_"+new Date().getTime() + ext;
		}		
		
		//파일 저장
		try {
			upload.transferTo(serverFile);  // 업로드된 파일을 지정된 이름으로 지정된 위치에 파일 저장 
			// 위 코드대로 안하면 byte[] 을 만들어서 while문 돌리면서 처음부터 끝까지 저장해야한다.
		} catch (Exception e) {
			savedFilename = null;
			e.printStackTrace();
		}
